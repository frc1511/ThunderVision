...
: images(_images), chessboard(_chessboard) {
  ...
  for (const auto& image : images) {
    // Load the image.
    img = cv::imread(image.string());

    // Convert the image to grayscale.
    cv::cvtColor(img, gray, cv::COLOR_BGR2GRAY);

    // Find the chessboard corners.
    bool found = cv::findChessboardCorners(gray, board_size, corners, cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_FILTER_QUADS);
    ...
    if (found) {
      ...
      cv::cornerSubPix(gray, corners, cv::Size(5, 5), cv::Size(-1, -1), cv::TermCriteria(cv::TermCriteria::EPS + cv::TermCriteria::MAX_ITER, 30, 0.1));
