...
: CameraStream(_props.props), usb_camera(props.name, _props.dev), dev(_props.dev) {
  init(&usb_camera);
...
: CameraStream(_props.props), http_camera(props.name, _props.url, cs::HttpCamera::HttpCameraKind::kMJPGStreamer), url(_props.url) {
  init(&http_camera);
}
